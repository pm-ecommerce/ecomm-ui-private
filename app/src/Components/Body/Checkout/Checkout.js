...
const Checkout = (props) => {
    ...
    const placeOrder = async (e) => {
        ...
        if (
            ...
            (+getCartTotal()).toFixed(2) !== payload.charges.reduce((acc, cur) => acc + +cur.amount, 0).toFixed(2)
        ) {
            setOpen(true);
            setPopUpMsg({
                isError : true,
            ...
            return fetch(`${ config.cartUrl }/api/cart/${ sessionId }`)
                ...
                .then((res) => {
                    ...
                    if (cardList.length > 0) {
                        ...
                        const total = res.data.reduce((sum, row) => sum + row.quantity * row.rate, 0);
                        first.amount = (total + (total * .07)).toFixed(2);
        ...
        <div className="success-page-container">
            <div className="success-card">
                ...
                <div className="scard-text-container">
                    <h3>OOPS!</h3>
                    <p>
                        Your cart is empty. Please add something to cart first.
                    </p>
                </div>
            </div>
        </div>
