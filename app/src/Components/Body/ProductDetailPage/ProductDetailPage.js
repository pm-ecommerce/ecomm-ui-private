...
const ProductDetailPage = (props) => {
    ...
    const updatePrice = (currentAttributes) => {
        const attrPrice = Object.values(currentAttributes).map(attr => attr.option.price).reduce((sum, row) => sum + row, 0);
        setPrice(attrPrice + product.price);
    };

    const handleSelect = (e, attribute) => {
        const option = attribute.options.find(option => option.id === +e.target.value);
            ...
            <div className="product-detail-box">
                <div className="product-detail-left">
                    ...
                    <div className="prodct-detail-img">
                        <img src={ getFirstImageUrl(product) } className="product-img"/>
                    </div>
                </div>
